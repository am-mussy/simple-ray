...
command -v id >/dev/null 2>&1 || fail "нужна команда id" 3
[[ "$(id -u)" == "0" ]] || fail "запусти: sudo bash start.sh" 3
...
[[ -f "${repo_dir}/go.mod" && -d "${repo_dir}/cmd/vpnctl" ]] || fail "запусти скрипт из каталога simple-ray" 3
...
printf 'Проверяю исходный код...\n' >&2
(cd -- "${repo_dir}" && "${go_binary}" test -count=1 ./...)
(cd -- "${repo_dir}" && "${go_binary}" vet ./...)

printf 'Собираю vpnctl для linux/%s...\n' "${source_arch}" >&2
...
[[ -s "${source_tmp}/vpnctl" ]] || fail "сборка vpnctl не создала исполняемый файл"
